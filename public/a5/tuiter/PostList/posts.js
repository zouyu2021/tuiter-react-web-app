export default [
    {
        icon:'https://pbs.twimg.com/profile_images/1442634650703237120/mXIcYtIs_400x400.jpg',
        author:'Elon Musk',
        handler:'@elonmusk',
        time:'23h',
        text:'Amazing show about ',
        hyperlink: '@Inspiration4X ',
        text2:'mission!',
        image:'https://occ-0-1068-1723.1.nflxso.net/dnm/api/v6/E8vDc_W8CLv7-yMQu8KMEC7Rrr8/AAAABSTVj5DHrlahOykYCHrcwL63gTkGzcXodPQgvrJXFqQMFg4Ktgtxmh_fGFJbsMF9BwmIbdW4L17YdJejrxJNwqyEnAP-.jpg?r=3d5',
        title:'Countdown: Inspiration4 Mission to Space | Netflix Official Site',
        text3:'From training to launch to landing, this all-access docuseries rides along with the Inspiration4 crew on the first all-civilian orbital space mission.',
        icon2:'<i class="fas fa-link"></i>&nbsp;',
        website:'netflix.com',
        replyNumber:'4.2k',
        retweetNumber:'3.5k',
        favoriteNumber:'37.5k',
        shareNumber:'',
    },

    {
        icon:'https://pbs.twimg.com/profile_images/966372680306868224/60wfGe9e_400x400.jpg',
        author:'New York Post',
        handler:'@nypost',
        time:'23h',
        text:'Grimes seen reading Karl Marx following split with world\'s richest man Elon Musk',
        hyperlink: 'trib.al/nx2Gfaq',
        text2: '',
        image:'https://pbs.twimg.com/media/FAuW2GlXIAIOBN7?format=jpg&name=small',
        title:'',
        text3:'',
        website:'',
        replyNumber:'965',
        retweetNumber:'2.4k',
        favoriteNumber:'4k',
        shareNumber:'',
    },
];
